...
### Durum Geçiş Diyagramı (Mermaid Formatında)
```mermaid
graph TD
    subgraph "Başlat ve Ayrıştır"
    ...
    subgraph "Ana İşlem Döngüsü"
    q3_find_bit -- "0" --> q4_move_to_equal{"Bit 0: Kaydır"};
    q3_find_bit -- "1" --> q4_move_to_equal{"Bit 1: Topla"};
    q3_find_bit -- "*" --> q9_cleanup{"Temizle"};
    end

  subgraph "Sonucu Yazma"
    q4_move_to_equal --> q6_write_result["Sonucu Yaz"];
    q4_move_to_equal --> q8_move_to_multiplier["Yazma Yoksa Dön"];
    q6_write_result --> q7_clear_tail["Kuyruk Temizle"];
    q7_clear_tail --> q8_move_to_multiplier["Bir Sonraki Bite Dön"];
    q8_move_to_multiplier --> q3_find_bit;
  end

  subgraph "Bitiş"
    q9_cleanup --> q_finalize_result["Son Kontrol"];
    q_finalize_result --> q_accept("Kabul Et");
  end

    style q_accept fill:#9f9,stroke:#333,stroke-width:2px
```

## 5. Program Ekran Görüntüleri

Program çalıştırıldığında, kullanıcıdan iki binary sayı girmesi istenir. Örnek olarak `11` ve `10` girilmiştir.
...
Ardından simülasyon başlar ve her adımı detaylı olarak gösterir:
...
İşlem bittiğinde, sonuç hem binary hem de decimal olarak gösterilir ve doğrulama yapılır.
